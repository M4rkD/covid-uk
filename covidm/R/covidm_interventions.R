...
... this doesn't work with split params/pops ...
cm_iv_apply <- function(parameters, iv, populations) {
  ...
  if (populations[1] < 0) {
    population_idxs <- seq_along(populations)
  }
  for (pi in population_idxs)
  {
    if (length(populations[[pi]]$schedule) == 0) {
      populations[[pi]]$schedule <- schedule
    } else {
